...
| v2.0.1 | 2026-06-30 | 修正全題庫與分類題庫的互斥選取邏輯，避免同時選取全題庫與其他分類。 |
| v2.0.1 | 2026-06-30 | Sidebar 新增「收藏題目」入口，可查看收藏清單並匯出收藏題目 JSON。 |
| v2.0.1 | 2026-06-30 | 選擇題首頁改為可捲動，視窗縮小時仍可查看最近刷題紀錄。 |
| v2.0.1 | 2026-06-30 | 調整首頁顯示：目前題庫文字加粗加大、匯入題庫按鈕移至右側並改為綠色。 |
| v2.0.1 | 2026-06-30 | 將選擇題首頁的「選擇大題」、「出題與顯示設定」、「選擇範圍」分成不同灰階背景區塊。 |
| v2.1 | 2026-07-01 | 新增收藏題目分類功能，收藏時可選擇分類，並可在系統設定新增收藏分類。 |
| v2.1 | 2026-07-01 | 收藏頁改為依分類查看題目，點選分類後以類似測驗結果頁的版型顯示題目內容、選項、正確答案與解析。 |
| v2.1 | 2026-07-01 | 調整收藏題目匯出按鈕為綠底白字，並更新程式顯示版本為 v2.1。 |
| v2.1.1 | 2026-07-02 | 作答畫面移除「儲存本題答案」按鈕，選擇答案後自動記錄作答狀態。 |
| v2.1.1 | 2026-07-02 | 調整作答畫面按鈕樣式：收藏本題改為橘色、結束測驗改為紅色，並隱藏作答區垂直捲軸。 |
| v2.1.1 | 2026-07-02 | 收藏題目時可直接新增收藏分類，收藏清單分類改為整列可展開或收合。 |
| v2.1.1 | 2026-07-02 | 收藏題目頁與系統設定頁新增滑鼠滾輪捲動支援。 |
...
- 匯入自訂 JSON 題庫
- 選擇題測驗，支援單選與多選
- 全題庫與分類題庫互斥選取，避免題目池設定衝突
- 題目與選項可設定亂數排序
- 作答時可切換上一題與下一題
- 可收藏目前題目，並選擇或新增要加入的收藏分類
- 可在系統設定新增收藏分類，預設分類為「已收藏的題目」
- 可從 Sidebar 查看收藏題目，依分類展開或收合題目內容並匯出收藏題目 JSON
- 交卷後統一檢查答案
...
- 可清除或匯出錯題紀錄
- 最近 10 次刷題紀錄
- 首頁可捲動，縮小視窗時仍可查看最近刷題紀錄
- 收藏題目頁與系統設定頁可使用滑鼠滾輪捲動
- 可下載本次題目 JSON
- 系統設定支援字體大小與主題模式
...
測驗進行中可點選「收藏本題」，並選擇要加入的收藏分類。

可從 Sidebar 的「收藏題目」頁依分類查看收藏清單，點選分類後可查看題目內容、全部選項、正確答案與解析，並可匯出成 JSON 檔案。

可在「系統設定」新增收藏分類。預設分類為：
...
```text
user_data/favorites/favorite_categories.json
```
